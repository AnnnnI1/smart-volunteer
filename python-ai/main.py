...
提供三个 AI 计算端点：
  ...
  POST /ai/feed           - Feed 流个性化推荐（行为向量融合 + DeepSeek 推荐语）
  POST /rag/chat          - RAG 活动发布智能问答
  POST /rag/diagnose      - RAG 活动内容合规诊断
  POST /rag/generate      - RAG 活动草稿生成
  POST /rag/rebuild-index  - RAG 知识库索引重建
  GET  /health           - 健康检查
"""
import os
import pathlib
...
from rag_service import build_index as rag_build_index, chat as rag_chat, diagnose_activity as rag_diagnose, generate_activity as rag_generate
from report_audit_service import analyze_report
...
def analyze_report_endpoint(req: ReportAuditRequest):
    """
    举报仲裁 AI 分析

    分析举报内容，返回风险等级、违规类型、证据强度、惩罚建议。
    用于管理员裁定时的 AI 辅助决策。
    """
    logger.info(f"Report audit: type={req.report_type}, category={req.category_code}, user={req.reported_user_id}")
    try:
        data = req.dict()
        result = analyze_report(data)
        return {"code": 200, "msg": "success", "data": result}
    except Exception as e:
        logger.error(f"Report audit error: {e}", exc_info=True)
        raise HTTPException(status_code=500, detail=str(e))
...
class RagDiagnoseRequest(BaseModel):
    ...
    requires_professional_skill: bool = False
    risk_note: str = ""
...
async def rag_chat_endpoint(req: RagChatRequest):
    """RAG 智能问答：检索知识库后调用 DeepSeek 生成回答"""
    logger.info(f"RAG chat: question={req.question[:50]}")
    try:
        result = rag_chat(req.question, req.history)
        return {"code": 200, "data": result}
    except Exception as e:
        logger.error(f"RAG chat error: {e}", exc_info=True)
        raise HTTPException(status_code=500, detail=str(e))

@app.post("/rag/diagnose")
async def rag_diagnose_endpoint(req: RagDiagnoseRequest):
    """RAG 活动内容诊断：结合表单内容和规则知识，输出结构化风险报告"""
    logger.info(f"RAG diagnose: title={req.title[:30]}")
    try:
        result = rag_diagnose(req.dict())
        return {"code": 200, "data": result}
    except Exception as e:
        logger.error(f"RAG diagnose error: {e}", exc_info=True)
        raise HTTPException(status_code=500, detail=str(e))

@app.post("/rag/generate")
async def rag_generate_endpoint(req: RagGenerateRequest):
    """RAG 活动草稿生成：根据一句话意图生成完整活动发布草稿"""
    logger.info(f"RAG generate: intent={req.intent[:30]}")
    try:
        result = rag_generate(req.intent, req.activity_type)
        return {"code": 200, "data": result}
    except Exception as e:
        logger.error(f"RAG generate error: {e}", exc_info=True)
        raise HTTPException(status_code=500, detail=str(e))

@app.post("/rag/rebuild-index")
async def rag_rebuild_index_endpoint():
    """重建 RAG 知识库索引（管理员用）"""
    logger.info("RAG rebuild index requested")
    try:
        result = rag_build_index()
        return {"code": 200, "message": "RAG 索引重建完成", "data": result}
    except Exception as e:
        logger.error(f"RAG rebuild index error: {e}", exc_info=True)
        raise HTTPException(status_code=500, detail=str(e))
...
async def warmup_models():
    ...
    def _warmup_rag():
        try:
            logger.info("正在构建 RAG 知识库索引...")
            rag_build_index()
            logger.info("RAG 知识库索引构建完成")
        except Exception as e:
            logger.warning(f"RAG 索引初始化失败（服务仍可用）: {e}")

    loop.run_in_executor(None, _warmup_transformer)
    loop.run_in_executor(None, _warmup_rag)
