...
def knn_match(required_skills: List[str], volunteers: List[Dict], top_k: int) -> List[Dict]:
    ...
    vol_texts = []
    for v in volunteers:
        skills_val = v.get("skills") or ""
        if isinstance(skills_val, list):
            skills_list = [s.strip() for s in skills_val if s]
        else:
            skills_list = [s.strip() for s in str(skills_val).split(",") if s.strip()]
    ...
    for rank, idx in enumerate(top_indices):
        vol = volunteers[idx]
        skills_val = vol.get("skills") or ""
        if isinstance(skills_val, list):
            vol_skills = [s.strip() for s in skills_val if s]
        else:
            vol_skills = [s.strip() for s in str(skills_val).split(",") if s.strip()]
