...
def _score_activities(user_vec, activities: List[Dict], noise_seed: int = 0) -> List[Dict]:
    """
    对活动池全局余弦相似度计算，返回带 feed_score 字段的列表（降序）
    noise_seed != 0 时加入小随机扰动，让刷新有新鲜感
    ...
    # 随机扰动：±5% 噪声，保持整体相关性但让刷新有新鲜感
    if noise_seed != 0:
        rng = np.random.default_rng(noise_seed)
        noise = rng.uniform(-0.05, 0.05, size=len(scores))
        scores = scores + noise
...
def feed_recommend(data: Dict[str, Any]) -> Dict[str, Any]:
    ...
    refresh_seed      = int(data.get("refresh_seed", 0))  # 刷新时传入时间戳种子
    ...
    try:
        ...
        scored = _score_activities(user_vec, activity_pool, noise_seed=refresh_seed)
    except Exception as e:
