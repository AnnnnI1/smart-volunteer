...
public interface ActivityService extends IService<VolActivity> {
    ...
    /** 管理员驳回活动（更新 auditStatus=2 并写明原因） */
    ResponseResult rejectActivity(Long id, String reason, String userId, String userRole);

    /** 管理员查询指定组织者名下进行中的活动（降级前风险提示用） */
    ResponseResult getActiveActivitiesByOrganizer(Long organizerId);
}
