...
public class ActivityController {
    ...
    public ResponseResult rejectActivity(@RequestHeader("X-User-Id") String userId,
                                         @RequestHeader("X-User-Role") String userRole,
                                         @PathVariable Long id,
                                         @RequestBody Map<String, String> body) {
        return activityService.rejectActivity(id, body.get("reason"), userId, userRole);
    }

    /** 管理员查询指定组织者名下进行中的活动（降级前风险提示用） */
    @GetMapping("/active-by-organizer/{organizerId}")
    public ResponseResult activeByOrganizer(@RequestHeader("X-User-Role") String userRole,
                                            @PathVariable Long organizerId) {
        if (Integer.parseInt(userRole) != 0) {
            return ResponseResult.errorResult(AppHttpCodeEnum.NO_OPERATOR_AUTH);
        }
        return activityService.getActiveActivitiesByOrganizer(organizerId);
    }
}
