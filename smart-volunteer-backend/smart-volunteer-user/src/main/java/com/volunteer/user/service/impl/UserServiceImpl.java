...
public class UserServiceImpl extends ServiceImpl<UserMapper, User> implements UserService {
    ...
    public ResponseResult listUsers(Integer role, Boolean applyOnly, Integer page, Integer size,
                                   String keyword, Integer status, Integer creditScoreMin,
                                   Integer creditScoreMax, String registerStart, String registerEnd) {
        ...
        if (registerStart != null && !registerStart.isEmpty()) {
            wrapper.ge(User::getCreatedAt, java.sql.Timestamp.valueOf(registerStart + " 00:00:00"));
        }
        if (registerEnd != null && !registerEnd.isEmpty()) {
            wrapper.le(User::getCreatedAt, java.sql.Timestamp.valueOf(registerEnd + " 23:59:59"));
        }
        ...
        java.util.List<UserAdminVo> voList = result.getRecords().stream()
                .map(u -> {
                    ...
                    Integer creditScore = userMapper.selectCreditScore(uid);
                    vo.setCreditScore(creditScore != null ? creditScore : 70);
                    ...
                    // banStatus 字段（从 User 实体获取）
                    vo.setBanStatus(u.getBanStatus());
                    return vo;
                })
                .collect(java.util.stream.Collectors.toList());

        // 信用分区间过滤（需要在 Java 层二次过滤，因为信用分在另一张表）
        if (creditScoreMin != null || creditScoreMax != null) {
            voList = voList.stream().filter(vo -> {
                int cs = vo.getCreditBalance() != null ? vo.getCreditBalance() : 0;
                if (creditScoreMin != null && cs < creditScoreMin) return false;
                if (creditScoreMax != null && cs > creditScoreMax) return false;
                return true;
            }).collect(java.util.stream.Collectors.toList());
        }
    ...
    public ResponseResult updateUserRole(Long targetId, Integer newRole, String operatorId, String operatorRole) {
        ...
        userMapper.updateById(user);
        // MyBatis-Plus 默认 NOT_NULL 更新策略，setAuditStatus(null) 不会更新到 DB
        // 必须用 UpdateWrapper 显式置空
    ...
    public ResponseResult getUserDetail(Long userId) {
        ...
        UserAdminVo vo = BeanCopyUtils.copyBean(user, UserAdminVo.class);
        Long uid = user.getId();
        Integer credit = userMapper.selectCreditBalance(uid);
        vo.setCreditBalance(credit != null ? credit : 0);
        Integer creditScore = userMapper.selectCreditScore(uid);
