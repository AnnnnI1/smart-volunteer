  ...
  <div>
    ...
    <el-card v-if="activeTab === 'all'" class="page-card" shadow="never">
      ...
      <el-table :data="list" v-loading="loading" border stripe style="width:100%">
        ...
        <el-table-column label="信用分" width="70">
          <template #default="{ row }">
    ...
    >
      <template v-if="applyDrawerRow">
        ...
        <div style="display:flex;gap:10px;flex-wrap:wrap;margin-bottom:20px">
          ...
          <div class="drawer-stat-card">
            <div class="drawer-stat-num" style="font-weight:bold" :style="{ color: (applyDrawerRow.creditScore ?? 70) < 40 ? '#f56c6c' : (applyDrawerRow.creditScore ?? 70) < 60 ? '#e6a23c' : '#67c23a' }">
              {{ applyDrawerRow.creditScore ?? 70 }}
            </div>
            <div class="drawer-stat-label">信用分</div>
          </div>
          <div class="drawer-stat-card">
    ...
    <el-dialog v-model="detailDialogVisible" :title="'用户详情 — ' + (detailRow?.nickname || detailRow?.username || '')" width="560px" destroy-on-close>
      <template v-if="detailRow">
        ...
        <div style="display:flex;gap:12px;flex-wrap:wrap;margin-bottom:16px">
          ...
          <div class="detail-stat-card">
            <div class="detail-stat-num" style="font-weight:bold" :style="{ color: (detailRow.creditScore ?? 70) < 40 ? '#f56c6c' : (detailRow.creditScore ?? 70) < 60 ? '#e6a23c' : '#67c23a' }">
              {{ detailRow.creditScore ?? 70 }}
            </div>
        ...
        <template v-else-if="detailRow.role !== 2">
          <div class="detail-section-title">组织者申请</div>
          <p style="color:#909399;font-size:13px;margin:0 0 12px 0">该用户未申请成为组织者</p>
        </template>

        <!-- 组织者活动列表：仅 role=2 显示 -->
        <template v-if="detailRow.role === 2">
          <div class="detail-section-title">发起的活动 ({{ detailOrgActivities.length }})</div>
          <el-empty v-if="detailOrgActivities.length === 0" description="暂无发起中的活动" :image-size="60" />
          <el-table v-else :data="detailOrgActivities" size="small" max-height="200" style="margin-bottom:8px">
            <el-table-column prop="id" label="ID" width="60" />
            <el-table-column prop="title" label="活动名称" min-width="150" show-overflow-tooltip />
            <el-table-column label="状态" width="80">
              <template #default="{ row }">
                <el-tag :type="activityStatusType(row.status)" size="small">{{ activityStatusLabel(row.status) }}</el-tag>
              </template>
            </el-table-column>
            <el-table-column label="审核" width="70">
              <template #default="{ row }">
                <el-tag v-if="row.auditStatus === 1" type="success" size="small">通过</el-tag>
                <el-tag v-else-if="row.auditStatus === 2" type="danger" size="small">驳回</el-tag>
                <el-tag v-else type="warning" size="small">待审</el-tag>
              </template>
            </el-table-column>
            <el-table-column label="时间" width="140">
              <template #default="{ row }">
                {{ row.startTime ? new Date(row.startTime).toLocaleString('zh-CN',{hour12:false}).replace(/\//g,'-').substring(0,16) : '-' }}
              </template>
            </el-table-column>
          </el-table>
        </template>
...
import { getActiveActivitiesByOrganizer } from '@/api/activity'
...
const openDetailDialog = (row) => {
  ...
  detailOrgActivities.value = []
  if (row.role === 2) {
    getActiveActivitiesByOrganizer(row.id).then(res => {
      detailOrgActivities.value = res.data || []
    }).catch(() => {})
  }
}
...
const activityStatusLabel = (s) => ({ 0: '未开始', 1: '进行中', 2: '已结束', 3: '已取消' }[s] ?? '-')
const activityStatusType = (s) => ({ 0: 'info', 1: 'success', 2: '', 3: 'danger' }[s] ?? 'info')
...
const changeRole = async (row, newRole) => {
  ...
  try {
    // 降级前检查是否有进行中的活动
    if (newRole === 1) {
      const res = await getActiveActivitiesByOrganizer(row.id)
      const activeList = res.data || []
      if (activeList.length > 0) {
        const activityNames = activeList.map(a => `「${a.title}」`).join('、')
        await ElMessageBox.confirm(
          `该用户名下有 ${activeList.length} 个活动仍在进行中：${activityNames}。\n降级后该用户将无法管理这些活动，确定继续降级？`,
          '降级风险提示',
          { confirmButtonText: '确定降级', cancelButtonText: '取消', type: 'warning' }
        )
      } else {
        await ElMessageBox.confirm(
