      ...
      <el-table :data="list" v-loading="loading" border stripe>
        ...
        <el-table-column label="审核结果" width="110">
          <template #default="{ row }">
            ...
            <el-tag v-if="row.isLatest" type="primary" size="small" effect="plain" style="margin-left:4px">最新</el-tag>
          </template>
        </el-table-column>
      ...
      <div v-if="currentRow">
        ...
        <el-descriptions :column="2" border size="small" style="margin-bottom:16px">
          ...
          <el-descriptions-item label="活动描述" :span="2">
            <span style="white-space:pre-wrap;font-size:12px;color:#606266;line-height:1.6">
              {{ getAuditDescription(currentRow.auditResult) || currentRow.activityDescription || '（无描述）' }}
            </span>
          </el-descriptions-item>
        ...
        <template v-if="getParsedData(currentRow.auditResult)">
          ...
          <template v-if="getParsedData(currentRow.auditResult).rule_engine">
            <el-alert title="规则引擎直接驳回，内容不合规" type="error" :closable="false" show-icon style="margin-bottom:12px" />
            <!-- 规则引擎信息 -->
            <el-descriptions :column="2" border size="small" style="margin-bottom:12px">
              <el-descriptions-item label="规则编号" :span="2">
                <el-tag type="danger" size="small">{{ getParsedData(currentRow.auditResult).rule_code || '未知' }}</el-tag>
              </el-descriptions-item>
              <el-descriptions-item label="规则名称" :span="2">
                {{ getParsedData(currentRow.auditResult).rule_name || '—' }}
              </el-descriptions-item>
              <el-descriptions-item label="驳回原因" :span="2">
                <span style="color:#f56c6c;font-weight:bold">{{ getParsedData(currentRow.auditResult).rule_message || getParsedData(currentRow.auditResult).suggestion || '—' }}</span>
              </el-descriptions-item>
          ...
          <!-- 风险原因（AI审核时展示） -->
          <template v-if="getParsedData(currentRow.auditResult).risk_reasons?.length && !getParsedData(currentRow.auditResult).rule_engine">
...
/** 从 auditResult JSON 中解析审核时的描述快照（优先展示） */
const getAuditDescription = (raw) => {
  const d = getParsedData(raw)
  return d?.activity_description || null
}
...
const fetchLogs = async () => {
  ...
  try {
    ...
    // 找出每个 activityId 的最新记录 id（id 最大的那条）
    const latestIdMap = {}
    rows.forEach(r => {
      if (!latestIdMap[r.activityId] || r.id > latestIdMap[r.activityId]) {
        latestIdMap[r.activityId] = r.id
      }
    })
    list.value = rows.map(r => ({ ...r, isLatest: r.id === latestIdMap[r.activityId] }))
