<template>
  <div>
    <el-card class="page-card" shadow="never">
      ...
      <el-table :data="list" v-loading="loading" border stripe>
        ...
        <el-table-column label="状态" width="110">
          <template #default="{ row }">
            ...
            <template v-else-if="row.auditStatus === 2">
              <el-tag type="danger" size="small">审核驳回</el-tag>
              <el-popover placement="top" :width="280" trigger="click">
                <template #reference>
                  <el-button type="danger" link size="small" style="margin-left:4px">查看原因</el-button>
                </template>
                <div style="font-size:13px;line-height:1.7;color:#303133">{{ row.rejectReason || '无' }}</div>
              </el-popover>
            </template>
        ...
        <el-table-column label="操作" width="450" fixed="right">
          <template #default="{ row }">
            <!-- 待审核：管理员可操作 -->
            <template v-if="row.auditStatus === 0 && isAdminUser">
              <el-button type="success" link size="small" @click="manualApproveActivity(row)">通过</el-button>
              <el-button type="danger" link size="small" @click="handleReject(row)">驳回</el-button>
            </template>
            <!-- 驳回状态：显示原因 + 组织者可编辑 -->
            <template v-else-if="row.auditStatus === 2">
              <el-tag type="danger" size="small" effect="plain">已驳回</el-tag>
              <el-button type="primary" link size="small" @click="openEdit(row)">编辑重提</el-button>
            </template>
            <!-- 已通过：正常操作 -->
    ...
    <!-- 举报志愿者对话框 -->
    <el-dialog v-model="volunteerReportVisible" title="举报志愿者" width="500px" :close-on-click-modal="false">
      <el-alert
      ...
      <template #footer>
        <el-button @click="volunteerReportVisible = false">取消</el-button>
...
import {
  ...
  adminApproveActivity, adminRejectActivity
} from '@/api/activity'
...
const editingRejected = ref(false)
...
const openEdit = (row) => {
  isEdit.value = true
  editingRejected.value = row.auditStatus === 2
  Object.assign(form, {
    id: row.id,
    title: row.title,
...
const handleSubmit = async () => {
  ...
  try {
    if (isEdit.value) {
      const res = await updateActivity(form.id, form)
      const d = res.data
      if (d && d.passed === false) {
        ElMessage.warning(d.message || '活动已保存，等待审核')
      } else {
        ElMessage.success(d?.message || '修改成功')
...
const handleReject = async (row) => {
  try {
    const reason = await ElMessageBox.prompt(
      `请填写驳回「${row.title}」的原因（将通知组织者）：`,
      '驳回活动',
      {
        confirmButtonText: '确认驳回',
        cancelButtonText: '取消',
        inputType: 'textarea',
        inputPlaceholder: '请输入驳回原因，如：活动内容与描述不符、涉及敏感信息等',
      }
    )
    if (!reason?.trim()) return
    await adminRejectActivity(row.id, reason)
...
// 举报志愿者
const volunteerReportVisible = ref(false)
...
const handleVolunteerReport = async () => {
  ...
  try {
    ...
    volunteerReportVisible.value = false
...
const openVolunteerReport = (reg) => {
  ...
  volunteerReportForm.activityId = regActivityId.value
  volunteerReportForm.reportType = ''
  volunteerReportForm.description = ''
  volunteerReportForm.evidenceInput = ''
  volunteerReportVisible.value = true
}
