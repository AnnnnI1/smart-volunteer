<template>
  <div>
      ...
      <el-col :span="10">
        <el-card class="profile-card" shadow="never">
          ...
          <el-row :gutter="12">
            ...
            <el-col :span="12" style="margin-top:12px">
              <div class="stat-box clickable" :class="creditHealthColor" @click="creditScoreDialog = true" title="点击查看信用分详情">
                <div class="stat-num" :style="{ color: creditHealthColor === 'red' ? '#f56c6c' : '#67c23a' }">
                  {{ userCreditScore?.creditScore ?? '—' }}
                </div>
                <div class="stat-label">信用分 <el-icon style="font-size:11px;vertical-align:middle"><View /></el-icon></div>
              </div>
            </el-col>
            <el-col :span="12" style="margin-top:12px">
              <div class="stat-box clickable" @click="myReportsDialog = true" title="点击查看我的举报记录">
                <div class="stat-num" style="font-size:24px">{{ myReportCount }}</div>
                <div class="stat-label">我的举报 <el-icon style="font-size:11px;vertical-align:middle"><View /></el-icon></div>
              </div>
            </el-col>
    ...
    <!-- 我的举报记录弹窗 -->
    <el-dialog v-model="myReportsDialog" title="我的举报记录" width="720px">
      <el-tabs>
        <el-tab-pane label="我举报的" name="submitted">
          <el-empty v-if="!mySubmittedReports.length" description="暂无举报记录" :image-size="60" />
          <el-table v-else :data="mySubmittedReports" size="small" max-height="400">
            <el-table-column label="活动" min-width="80">
              <template #default="{row}">
                <el-button v-if="row.activityId" link type="primary" size="small"
                  @click="myReportsDialog=false; router.push(`/homepage/activity/${row.activityId}`)">
                  #{{ row.activityId }}
                </el-button>
                <span v-else style="color:#909399">—</span>
              </template>
            ...
            <el-table-column label="被举报人" min-width="110">
              <template #default="{row}">
                <span>{{ row.reportedNickname || row.reportedUsername || (row.reportedUserId ? '用户#' + row.reportedUserId : '-') }}</span>
              </template>
            </el-table-column>
            <el-table-column label="类型" width="110">
              <template #default="{row}">{{ reportDisplayType(row) }}</template>
            </el-table-column>
        ...
        <el-tab-pane label="被举报我的" name="received">
          <el-empty v-if="!myReceivedReports.length" description="暂无被举报记录" :image-size="60" />
          <div v-else>
            <el-alert
              v-for="r in myReceivedReports"
              :key="r.id"
                :title="reportDisplayType(r) + ' - ' + reportStatusLabel(r.status)"
              :type="r.status >= 3 ? 'danger' : 'warning'"
              :closable="true"
            ...
            >
              <template #default>
                ...
                <div style="font-size:12px;color:#909399;margin-top:4px">
                  举报人：{{ r.reporterNickname || r.reporterUsername || ('用户#' + r.reporterId) }} | 时间：{{ fmtTime(r.createTime) }}
                </div>
              </template>
            </el-alert>
          </div>
        ...
        <el-tab-pane label="我的申诉" name="appeals">
          <el-empty v-if="!myAppeals.length" description="暂无申诉记录" :image-size="60" />
          <el-table v-else :data="myAppeals" size="small" max-height="400">
            <el-table-column label="关联举报" width="90">
              <template #default="{row}">
                <span>举报#{{ row.reportId }}</span>
              </template>
            </el-table-column>
            <el-table-column prop="appealContent" label="申诉内容" min-width="160" show-overflow-tooltip />
            <el-table-column label="申诉状态" width="100">
              <template #default="{row}">
                <el-tag :type="appealStatusType(row.status)" size="small">{{ appealStatusLabel(row.status) }}</el-tag>
              </template>
            </el-table-column>
            <el-table-column prop="adminReply" label="管理员回复" min-width="130" show-overflow-tooltip />
            <el-table-column label="时间" min-width="130">
              <template #default="{row}">{{ fmtTime(row.createTime) }}</template>
            </el-table-column>
          </el-table>
      ...
      <template #footer>
        <el-button @click="myReportsDialog = false">关闭</el-button>
      </template>
    </el-dialog>

    <!-- 信用分详情弹窗 -->
    <el-dialog v-model="creditScoreDialog" title="我的信用分详情" width="480px">
      <div v-if="userCreditScore">
        <el-descriptions :column="1" border>
          <el-descriptions-item label="信用分">
            <span style="font-size:28px;font-weight:bold" :style="{ color: creditHealthColor === 'red' ? '#f56c6c' : '#67c23a' }">
              {{ userCreditScore.creditScore }}
            </span>
          </el-descriptions-item>
          <el-descriptions-item label="信用等级">{{ userCreditScore.creditLevel || '正常' }}</el-descriptions-item>
          <el-descriptions-item label="组织者等级">
            <el-tag v-if="userCreditScore.organizerLevel > 0" type="warning" size="small">
              {{ { 1: '新晋', 2: '铜牌', 3: '银牌', 4: '金牌' }[userCreditScore.organizerLevel] || '新晋' }}
            </el-tag>
            <span v-else style="color:#909399">普通志愿者</span>
          </el-descriptions-item>
          <el-descriptions-item label="违规次数">
            {{ userCreditScore.totalViolations || 0 }} 次
          </el-descriptions-item>
          <el-descriptions-item label="被举报次数">
            {{ userCreditScore.totalReports || 0 }} 次
          </el-descriptions-item>
          <el-descriptions-item label="活动创建限制">
            <span v-if="userCreditScore.activityLimitUntil">
              <span style="color:#f56c6c">
                限制中（截止 {{ fmtTime(userCreditScore.activityLimitUntil) }}）
              </span>
            </span>
            <span v-else style="color:#67c23a">无限制</span>
          </el-descriptions-item>
          <el-descriptions-item label="账号状态">
            <template v-if="userCreditScore.banType === 1">
              <el-tag type="warning" size="small">临时封禁</el-tag>
              <span style="color:#909399;font-size:12px;margin-left:4px">
                截止 {{ fmtTime(userCreditScore.banUntil) }}
              </span>
            </template>
            <template v-else-if="userCreditScore.banType === 2">
          ...
          <el-descriptions-item v-if="userCreditScore.lastViolationAt" label="最近违规">
            {{ fmtTime(userCreditScore.lastViolationAt) }}
          </el-descriptions-item>
        </el-descriptions>
        <el-alert
          v-if="userCreditScore.creditScore < 60"
          title="您的信用分较低，请注意规范行为。分数将随时间自动恢复。"
          type="warning"
          :closable="false"
          show-icon
        ...
        />
        <el-alert
          v-else-if="userCreditScore.creditScore >= 90"
          title="您的信用分优秀，继续保持！"
          type="success"
          :closable="false"
          show-icon
      ...
      <template #footer>
        <el-button @click="creditScoreDialog = false">关闭</el-button>
      </template>
    </el-dialog>
  </div>
</template>
...
import { listMyReports, listMyReceivedReports, listMyAppeals, getUserCreditScore } from '@/api/report'
...
const userCreditScore = ref(null)
const mySubmittedReports = ref([])
const myReceivedReports = ref([])
const myAppeals = ref([])
const myReportCount = computed(() => mySubmittedReports.value.length)

const creditHealthColor = computed(() => {
  const s = userCreditScore.value?.creditScore ?? 100
  return s < 60 ? 'red' : 'green'
})

const fetchUserCreditScore = async () => {
  try {
    const res = await getUserCreditScore(authStore.userInfo?.id)
    userCreditScore.value = res.data
  } catch (e) { /* 服务未启动或无数据时静默降级 */ }
}
...
const fetchMyReports = async () => {
  try {
    ...
    const [submittedRes, receivedRes] = await Promise.all([
      listMyReports({ page: 1, size: 100 }),
      listMyReceivedReports({ page: 1, size: 100 })
    ])
    mySubmittedReports.value = submittedRes.data?.rows || []
    myReceivedReports.value = receivedRes.data?.rows || []
  } catch (e) {}
}

const fetchMyAppeals = async () => {
  try {
    const res = await listMyAppeals({ page: 1, size: 100 })
    myAppeals.value = res.data?.rows || []
  } catch (e) {}
}

const reportTypeLabel = (t) => ({
  fake_activity: '虚假活动', fraud_charge: '欺诈收费',
  harassment: '骚扰虐待', violation_content: '违规内容',
  other: '其他问题', AR: '活动举报', VR: '志愿者违规'
}[t] ?? t)

const reportStatusLabel = (s) => ({ 0: '待受理', 1: '处理中', 2: '已采纳', 3: '已驳回', 4: '已撤销' }[s] ?? s)
const reportStatusType = (s) => ({ 0: 'info', 1: 'warning', 2: 'success', 3: 'danger', 4: 'info' }[s] ?? 'info')

// Use categoryCode for display (preserves original type like fake_activity), fallback to reportType
const reportDisplayType = (row) => reportTypeLabel(row.categoryCode || row.reportType)

const appealStatusLabel = (s) => ({ 0: '待处理', 1: '已通过', 2: '已驳回' }[s] ?? s)
const appealStatusType = (s) => ({ 0: 'info', 1: 'success', 2: 'danger' }[s] ?? 'info')
...
// ── 角色变更检测（升级/降级均强制重新登录以刷新JWT） ─────────────────
const roleUpgraded = ref(false)
const roleDowngraded = ref(false)
...
const checkMyStatus = async () => {
  try {
    ...
    // 角色降级：组织者被管理员降为志愿者，JWT已过期，强制退出
    if (dbUser.role === 1 && current.role === 2) {
      roleDowngraded.value = true
      ElMessage.warning('您的角色已被管理员更新，请重新登录')
      setTimeout(() => {
        authStore.logout()
        router.push('/login')
      }, 1500)
...
.stat-box.red    { background: #fef0f0; }
