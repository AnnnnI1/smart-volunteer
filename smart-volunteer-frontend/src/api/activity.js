...
// 管理员驳回活动
export function adminRejectActivity(id, reason) {
  return request({ url: `/activity/${id}/reject`, method: 'put', data: { reason } })
}

// 管理员查询指定组织者名下进行中的活动（降级前风险提示用）
export function getActiveActivitiesByOrganizer(organizerId) {
  return request({ url: `/activity/active-by-organizer/${organizerId}`, method: 'get' })
}
